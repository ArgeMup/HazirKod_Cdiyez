...
namespace ArgeMup.HazirKod
{
    ...
    public class Ayarlar_Xml_ : IDisposable 
    {
        ...
        public Ayarlar_Xml_(out bool Sonuç, string AyarlarİçinParola = "", string AyarlarDosyası = "", bool Izin_AltDallarıdaKarıştır = false, int Süre_DeğişiklikleriKaydetmeAralığı_Sn = 30, int Süre_KaynaklarıBoşaltmaAralığı_Dk = 30)
        {
        ...
        {
            string çıktı = Depo_Xml.Oku(Xml, Adı, BulunamamasıDurumundakiİçeriği);
        ...
        public bool Yaz_AltDal(ref string Xml, string Adı, string İçeriği)
        {
            ...
            return Depo_Xml.Yaz(ref Xml, Adı, İçeriği);
        }
      
        ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        
        public List<Depo_Xml.Biri> Listele()
        {
            List<Depo_Xml.Biri> Liste = new List<Depo_Xml.Biri>();
            try
            {
                if (!EtkinMi()) throw new Exception();
                ...
                foreach (XmlNode Par in AyarlarDalı.ChildNodes)
                {
                    Depo_Xml.Biri Yeni = new Depo_Xml.Biri();
                    Yeni.Adı = Par.InnerText;
        ...
        public List<Depo_Xml.Biri> Listele_AltDal(string Xml)
        {
            List<Depo_Xml.Biri> Liste = Depo_Xml.Listele(Xml);
            if (string.IsNullOrEmpty(Parola) || !AltDallarıdaKarıştır) return Liste;

            List<Depo_Xml.Biri> Liste_İki = new List<Depo_Xml.Biri>();
            foreach (Depo_Xml.Biri Biri in Liste)
            {
                Liste_İki.Add(new Depo_Xml.Biri(Biri.Adı, Karmaşıklaştırma.Düzelt(Biri.İçeriği, Parola)));
            }
            return Liste_İki;
        }

        public bool ListeyiEkle(List<Depo_Xml.Biri> Liste)
        {
            try
            {
        ...
        public bool ListeyiEkle_AltDal(ref string Xml, List<Depo_Xml.Biri> Liste)
        {
            if (string.IsNullOrEmpty(Parola) || !AltDallarıdaKarıştır) return Depo_Xml.ListeyiEkle(ref Xml, Liste);

            List<Depo_Xml.Biri> Liste_İki = new List<Depo_Xml.Biri>();
            foreach (Depo_Xml.Biri Biri in Liste)
            {
                Liste_İki.Add(new Depo_Xml.Biri(Biri.Adı, Karmaşıklaştırma.Karıştır(Biri.İçeriği, Parola)));
            }
            return Depo_Xml.ListeyiEkle(ref Xml, Liste_İki);
        }
